...
Use `spinClassName` to override the animation, such as the duration or easing:

```tsx
<SpinningCircularText
  text="Built with care by ncdai • "
  spinClassName="duration-[10s]"
/>
```

### Per-character rendering

Use `renderChar` to control how each character is rendered, e.g. to animate each
one with a `motion.span`. The returned node is placed inside a positioned wrapper,
so you only need to render the visible content:
